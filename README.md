...
아이디는 추후 modifier를 제거할 때 구분하기 위해 사용하는데 이때 아이디를 modifier를 보낼때 설정하면 사용할때 마다 서로 다른 아이디를 구분해서 설정해야하는 번거로움이 있기 때문에 그렇게 하지 않고 modifier를 적용하면서 동시에 handle에 아이디를 설정하여 이를 반환하고 보관하고 있다가 modifier를 제거하는데 사용하도록 하였습니다.

적용된 modifier는 attribute를 계산할때 사용됩니다.

```c#
// base value와 modifier의 value를 합쳐 최종적인 값을 구하는 함수
...
2차 attribute는 오직 플레이어에게만 유효하며 에너미의 경우 1차 attribute를 통해 2차 attribute를 설정하면 밸런싱 작업이 매우 힘들기 때문에 직접 2차 attribute를 설정하는 방식으로 구현하였습니다.
...
이렇게 계산기에 설정이 완료되면 계산기를 생성할 대 map을 통해 attribute별로 의존하고 있는 attribute를 설정합니다.
...
```
Base ScriptableObject의 예시로 이를 상속받아 추가적으로 필요한 요소를 추가하여 사용합니다.

해당 SO 클래스는 ASC에서 보관하며 ASC는 `CreateInstance`함수를 통해 abiltiy 로직이 담긴 클래스를 보관할 수 있게 됩니다.
...
```
이후 id를 통해 ability를 찾아 실행시킵니다. `ActivateAbility`함수에 spec과 asc를 보내는데 이를 통해 각각의 ability는 자신의 정보가 담긴 SO클래스에 접근하여 대미지나 이펙트를 설정하며 owner의 정보를 찾아 적이 누구인지나 현재 owner 위치등을 알 수 있게 됩니다.
...
```
애니메이션은 크게 실행 중 이벤트를 받아 어떤 로직을 실행하거나 애니메이션이 종료되거 어떤 로직이 실행되어야 할 것입니다.

코루틴을 사용하여 애니메이션의 종료 시점을 정확히 판별하고 콜백 함수가 실행되도록 하였습니다.
...
단순히 애니메이션 실행 및 종료가 아니라 event도 받아야하기 때문에 추가적인 함수를 만들었습니다.
