...
- [4. 핵심 기능 및 구현 내용](#4-핵심-기능-및-구현-내용)
  ...
  - [4.2 Attribute(능력치)](#42-attribute능력치)
    ...
    - [4.2.3 2차 attribute](#423-2차-attribute)
    - [4.2.4 AttributeCalculator](#424-attributecalculator)
    - [4.2.5 전략 패턴을 통한 attribute 설정](#425-전략-패턴을-통한-attribute-설정)
    - [4.2.6 defendancy 설정](#426-defendancy-설정)
    - [4.2.7 DFS 통한 순환 참조 예방](#427-dfs-통한-순환-참조-예방)
  - [4.3 Ability (스킬)](#43-ability-스킬)
    - [4.3.1 Ability 구현](#431-ability-구현)
    - [4.3.2 Abiltiy 데이터와 로직 설정](#432-abiltiy-데이터와-로직-설정)
...
```
modifier가 적용되거나 Base Value가 변경되면 `Recalculate`함수를 사용하여 current value를 설정합니다.
...
#### 4.2.4 AttributeCalculator
`AttributeCalculator`는 2차 Attribute를 계산하기 위해 사용하는 클래스입니다.
...
```
베이스가 되는 계산 클래스로 Attribute별 계산 클래스는 `CalculateAttribute`함수를 오버라이드하여 자신만의 계산식을 구현하게 됩니다.
...
```
실제 물리공격력을 계산하는 클래스의 로직으로 힘과 민첩을 가져와 계산 후 값을 반환하는 로직을 작성한 것을 볼 수 있습니다.

#### 4.2.5 전략 패턴을 통한 attribute 설정
attribute 타입별로 서로 다른 계산 클래스를 사용해야하고 이를 위해 전략패턴을 적용하였습니다.

```c#
// Key: 2차 attribute, value: Key에 맞는 계산 클래스
...
```c#
// Attribute Set 클래스 내부에 존재하는 계산기 생성 함수
...
```
이후 각각 계산기 클래스들은 map에 타입 별로 저장됩니다.
...
이렇게 map을 구성하고 1차 attribute가 변하면 이 map을 확인하여 2차 attribute도 설정합니다.
...
#### 4.3.1 Ability 구현
AbilityLogicBase 클래스를 상속 받아 어빌리티를 구현합니다. 추상화를 통해 ASC는 각각의 어빌리티의 실제 구현을 알지 못하여도 Abiliy가 실행이 가능하게 하였습니다.

[🔝 Top](#)

#### 4.3.2 Abiltiy 데이터와 로직 설정
![파이어볼 SO](GuideImg/fierball%20ability%20SO.png)

ability에 필요한 데이터는 ScriptableObject에서 설정됩니다. 이를 통해 동일한 ability라고 하더라도 전혀 다른 대미지와 효과를 주어 다른 ability처럼 보이는 것이 가능하게 하였습니다.
...
```
ScriptableObject의 베이스 클래스로 `CreateInstance` 함수가 존재하는데 이를 통해 SO는 자신의 실행 로직이 담긴 인스턴스를 생성하고 반환하는 것이 가능합니다. 
...
```
자식 클래스의 구현으로 `return new NormalAttackAbility();`를 반환하는 것을 볼 수 있는데 이렇게 자식 클래스에서 실제 인스턴스를 반환하는 팩토리 메서드 패턴을 사용하여 새로운 Ability 추가 시 기존 코드를 수정하지 않도록 설계했습니다.
...
## 5. 문제 해결 및 방법
유니티 프로젝트를 처음 만들면서 가장 고민했던 점은 구조를 어떻게 잡을지 하는 부분이었습니다.

언리얼엔진의 경우 대부분의 클래스들이 엔진에서 미리 만들어진 클래스를 상속받아 사용하지만 유니티의 경우 어떤 클래스를 만들지 완전히 자유로웠고 그렇기에 고민이 더 컸습니다.

사실 단순히 기능만 구현하는 것이 목표라면 고민할 필요가 없었겠지만 체계적이고 재사용이 가능하며 유지보수가 용이한 프로젝트를 구현하고 싶었고 그렇기에 처음에 구조를 잘 잡아야 나중에 고생을 안한다는 강박관념이 생기기도 하였습니다.

하지만 이렇게 고민만 하니 프로젝트 제작 시간만 늘어났고 그렇기에 차라리 먼저 만들고 나중에 구조를 잡자라는 생각을 하였습니다.

이렇게 하니 오히려 더 구조가 명확해지고 리팩토링을 거듭하면서 프로그래밍 실력이 늘어나게 되었습니다.

실제 현업에서는 처음부터 구조를 잘 잡으면 좋지만 그에 못지 않은게 리팩토링 능력을 키우는 것이라고 생각하며 이 프로젝트를 통해 실력을 더 늘릴 수 있었습니다.
